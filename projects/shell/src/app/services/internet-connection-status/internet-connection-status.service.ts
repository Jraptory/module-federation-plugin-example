...
export class InternetConnectionStatusService implements OnDestroy {
  ...
  constructor(private http: HttpClient, @Inject(ConnectionServiceOptionsToken) @Optional() options: ConnectionServiceOptions) {
    this.serviceOptions = _.defaults({}, options, InternetConnectionStatusService.DEFAULT_OPTIONS);
